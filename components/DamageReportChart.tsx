...
import { BarChart2, Monitor, Building2 } from 'lucide-react';
...
const DamageReportChart: React.FC<DamageReportChartProps> = ({ reports }) => {
  ...
  const CategorySection = ({ title, data, maxCount, color, Icon }: { title: string, data: AggregatedDamage[], maxCount: number, color: string, Icon: React.FC<any> }) => {
    ...
    const colorClasses = {
        violet: { text: 'text-violet-600', bg: 'bg-violet-500', bgLight: 'bg-violet-100' },
        amber: { text: 'text-amber-600', bg: 'bg-amber-500', bgLight: 'bg-amber-100' },
    };
            ...
            <div className="space-y-3">
                {data.map(item => (
                    <div key={item.type} className="text-xs">
                        ...
                        <div className={`w-full h-2 rounded-full ${theme.bgLight}`}>
                            <div 
